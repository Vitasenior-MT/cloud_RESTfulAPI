Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    # mysql -u root -p123qwe -h localhost -e "DROP DATABASE IF EXISTS node;DROP DATABASE IF EXISTS test;CREATE DATABASE IF NOT EXISTS node;CREATE DATABASE IF NOT EXISTS test;CREATE USER IF NOT EXISTS 'api'@'localhost' IDENTIFIED BY '123qwe';GRANT ALL PRIVILEGES ON node.* TO 'api'@'localhost';GRANT ALL PRIVILEGES ON test.* TO 'api'@'localhost'; FLUSH PRIVILEGES;"
