Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    mysql -u root -p123qwe -h localhost -e "CREATE DATABASE IF NOT EXISTS node;CREATE USER IF NOT EXISTS 'api'@'localhost' IDENTIFIED BY '123qwe';GRANT ALL PRIVILEGES ON node.* TO 'api'@'localhost'; FLUSH PRIVILEGES;"
    # mysql -u root -p123qwe -h localhost node < ../../vagrant/db_dump.sql
    
    # to get in mysql terminal
    # mysql -u api -p123qwe node
